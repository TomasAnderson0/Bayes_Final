...
ggplot(posteriorcomp) + stat_summary(aes(y = variable, x = b1), fun.data = mean_sdl, color = "#057057") + 
...
posteriorcomp2 <- data.frame(extract(modeloregcomp2, c("beta1", "beta2", "beta3", "beta4", "beta5")))
posteriorcomp2 <- data.frame(b1 = c(posteriorcomp2$beta1, posteriorcom2p$beta2, posteriorcomp2$beta3,
                                   posteriorcomp2$beta4, posteriorcomp2$beta5, posteriorcomp2$beta6,
                                   posteriorcomp2$beta7))
posteriorcomp2 <- posteriorcomp2 %>% 
  mutate(variable = c(rep("Edad", 5400), rep("Sexo", 5400), rep("Resonador GE", 5400), rep("Resonador Philips", 5400), rep("Intensidad", 5400)))

ggplot(posteriorcomp2) + stat_summary(aes(y = variable, x = b1), fun.data = mean_sdl, color = "#057057") + 
  geom_vline(xintercept = 0, linetype = 2) + labs(y = "Variable", x = "Coeficiente") + theme_minimal()
...
posteriorcomp3 <- data.frame(extract(modeloregcomp3, c("beta1", "beta2", "beta3", "beta4", "beta5")))
...
ggplot(posteriorcomp3) + stat_summary(aes(y = variable, x = b1), fun.data = mean_sdl, color = "#057057") + 
  geom_vline(xintercept = 0, linetype = 2) + labs(y = "Variable", x = "Coeficiente") + theme_minimal()
...
posteriorcomp4 <- data.frame(extract(modeloregcomp4, c("beta1", "beta2", "beta3", "beta4", "beta5")))
...
ggplot(posteriorcomp4) + stat_summary(aes(y = variable, x = b1), fun.data = mean_sdl, color = "#057057") + 
  geom_vline(xintercept = 0, linetype = 2) + labs(y = "Variable", x = "Coeficiente") + theme_minimal()
