...
ggplot(datan) + geom_line(aes(x = x , y = y)) + labs(x = "Parametro", y = "Densidad", caption=" Priors univariados para los coeficientes") + scale_x_continuous(breaks = -2:2) + theme_minimal(base_size = 20) + theme(plot.caption = element_text(hjust = .5, color = "#555555"))
...
```{r, fig.align='center'}
load("reglin.RData")
reglogplot + labs(caption = "Posteriors de los parametros para cada modelo univariado") + theme(plot.caption = element_text(hjust = .5, color = "#555555"))
```
...
reglogplot1 + theme(plot.caption = element_text(hjust = .5, color = "#555555")) +
  labs(caption = "Posteriors de los parametros para el modelo conjunto")
```
...
```{r}
load("ic_comp.RData")
ic_comp + theme_minimal(base_size = 20)  + theme(plot.caption = element_text(hjust = .5, color = "#555555")) + labs(caption = "Diferencia de ELPPD de los modelo con el de VHI y ECSF")
```
...
::: {style="font-size: 87%;"}
Se sabe que existe una fuerte relación entre la morfologia cerebral y la presencia de enfermedades neurodegenerativas. Ademas, la morfologia del cerebro se ve reducida en tamaño en consecuencia del aumento de la edad. 
